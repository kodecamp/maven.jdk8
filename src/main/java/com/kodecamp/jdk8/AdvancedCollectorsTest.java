...
public class AdvancedCollectorsTest {
  ...
  public static void main(String args[]) {
    ...
    /* partitionPersonList(people, 30); */
    partitionPersonListWithNameOnly(people, 30);
  }
  ...
  private static void partitionPersonList(List<Person> people, int filterAge) {
    Map<Boolean, List<Person>> map = people.stream()
        .collect(partitioningBy(p -> p.age == filterAge));
    System.out.println(map);

  }

  private static void partitionPersonListWithNameOnly(List<Person> people,
      int filterAge) {
    Map<Boolean, List<String>> map = people.stream()
        .collect(partitioningBy(p -> p.age == filterAge,
